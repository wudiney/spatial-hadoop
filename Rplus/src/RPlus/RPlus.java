...
public class RPlus<T> {
	...
	/**
	 * Builds a new R+Tree using default parameters: maximum 50 entries per node
	...
	/**
	 * Searches the R+Tree for objects overlapping with the given rectangle.
	 * 
	 * @param coords
	...
	/**
	 * Inserts the given entry into the R+Tree, associated with the given
	 * rectangle.
	 * 
	 * @param coords
	...
	public void insert(float[] coords, float[] dimensions, T entry) {
		...
		for (Node leaf : leaves) {
			leaf.children.add(e);
			e.parent = leaf;
			if (leaf.children.size() > maxEntries) {	//if leaf node do not have enough room
...
//tighten. If parent node overflow, split parent node (upward recursively). otherwise, tighten parent node upward.
	...
	public Node[] splitNode(Node n) {
		...
		float[] cutInfo = evaluate(n.children);	//Look for a cutline (based on x or y axis) which cuts/intersects less nodes
		...
		for (Node c : cc) {
			Node[] result = assign(c, cutInfo);	//locate subnodes into two split nodes
	...
	public Node[] assign(Node n, float[] cutInfo) {
		...
		if (result == 1) {
			...
			Node[] nn = partition(n, cutInfo);	//divided the node by cutline
	...
	private void chooseLeaves(RPlus<T>.Node n, RPlus<T>.Entry e,
		...
		if (!overlap) {
			...
			for (RPlus<T>.Node c : n.children) {
				...
				if (inc < minInc) {
					minInc = inc;
					next = c;	//choose the node require least enlargement
				} else if (inc == minInc) {
					float curArea = 1.0f;
					float thisArea = 1.0f;
